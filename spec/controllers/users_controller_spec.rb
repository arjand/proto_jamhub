...
describe UsersController do
  ...
  context 'POST create' do 
    context 'with valid parameters' do
      let(:valid_attributes) {{:email => "plowry@scu.edu", :password => "abc1234", :password_confirmation => 'abc1234', :jam_name => 'plowry'}}
    ...
    context 'with invalid parameters' do
      let(:invalid_attributes) {{:email => "", :password => "", :password_confirmation => '', :jam_name => ''}}
  ...
  context 'PUT update' do
    ...
    context 'with authorized session' do
      context 'with valid parameters' do 
        let(:valid_attributes) {{:email => user.email, :jam_name => 'new_name', :password => user.password, :password_confirmation => user.password_confirmation}}
        let(:valid_parameters) {{:id => user.id, :user => valid_attributes}}
        before {put :update, valid_parameters, 'user_id' => user.id}

        it 'updates the user attributes' do
          User.find(user.id).jam_name.should eq valid_attributes[:jam_name]
        end
      ...
      context 'with invalid parameters' do
        let(:invalid_attributes) {{:email => '', :jam_name => '', :password => '', :password_confirmation => ''}}
